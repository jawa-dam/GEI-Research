#!/usr/bin/env python3
"""GEI-Research V1.0.9 validation engine."""
...
def main():
    ...
    for path in all_records():
        ...
        try:
            schema=load_json(SCHEMA/schema_name);validator=Draft202012Validator(schema, resolver=RefResolver(schema_path.as_uri() if False else (SCHEMA/schema_name).as_uri(),schema))
    ...
    for rel,obj in parsed.items():
        ...
        if obj.get("type")=="interpretation" and not(obj.get("source_ids") or obj.get("evidence_ids") or obj.get("supports_ids")):warnings.append(f"{rel}: interpretation has no explicit source/evidence/support linkage")
    ...
    print("GEI-Research Validation Engine V1.0.9");print(f"Records scanned: {len(parsed)}");print(f"Errors: {len(errors)}");print(f"Warnings: {len(warnings)}")
