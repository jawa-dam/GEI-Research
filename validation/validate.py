...
def walk_refs(value,key=None):
    if isinstance(value,dict):
        ...
        if ID_RE.fullmatch(value) or PRO_RE.fullmatch(value) or RUN_RE.fullmatch(value) or RPL_RE.fullmatch(value) or RNR_RE.fullmatch(value):yield value
def main():
    ...
    for path in all_records():
        ...
        records[rid]=rel
        if not (ID_RE.fullmatch(rid) or PRO_RE.fullmatch(rid) or RUN_RE.fullmatch(rid) or RPL_RE.fullmatch(rid) or RNR_RE.fullmatch(rid)):errors.append(f'{rel}: invalid id format: {rid}')
    ...
    for rel,obj in parsed.items():
        ...
        if obj.get('type')=='reproducibility':
            ...
            for run in obj.get('reproduction_runs',[]):
                ...
                if run.get('status') in {'completed','failed','inconclusive'} and any(s.get('status')=='not_run' for s in run.get('step_results',[])):errors.append(f'{rel}: terminal reproduction run contains not_run steps')
